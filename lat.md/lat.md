...
[[crates/transport_mio/src/pool.rs#SharedVecPool]] is the `Arc`-wrapped handle backends and callers share; it wraps [[crates/transport_mio/src/pool.rs#VecPool]] which owns the `parking_lot::Mutex<Vec<Vec<u8>>>` free list plus configured `slab_size`. `acquire(len)` returns `None` when `len > slab_size` or when the free list is drained, giving a natural backpressure signal.

[[crates/transport_mio/src/pool.rs#VecSlab]] is the owned handle. It carries `Arc<VecPool>`, a length, and its own `Vec<u8>`. `Drop` clears + resizes the buffer, pushes it back onto the free list, and decrements the `in_use` counter. `AsRef<[u8]>` returns the filled slice up to `len`.
...
[[crates/transport_mio/src/udp.rs#UdpTransport]] builds a non-blocking `socket2::Socket`, applies reuse + `SO_RCVBUF`/`SO_SNDBUF` via [[crates/transport_mio/src/udp.rs#apply_socket_opts]], binds, hands the fd to `mio::net::UdpSocket::from_std`, and registers it on a fresh `mio::Poll`. `poll_ready(timeout)` drains readiness into an internal `readable` flag; `try_recv` calls `recv_from` and stores the datagram for `peek_frame`.

[[crates/transport_mio/src/udp.rs#UdpFrame]] is the borrowed view. Raw UDP has no sequencing so `sequence()` and `stream_id()` return zero; protocol crates (moldudp, custom framers) layer sequencing on top.
...
[[crates/transport_mio/src/tcp.rs#TcpTransport]] opens the stream, applies `SO_RCVBUF`/`SO_SNDBUF` via [[crates/transport_mio/src/tcp.rs#apply_tcp_socket_opts]], registers on a fresh `mio::Poll`, then blocks in `wait_connect` until the initial writable event lands. `try_recv` reads one chunk per call; a zero-byte read is surfaced as `UnexpectedEof` so the caller sees graceful peer close. `send` loops on `WouldBlock` with a short `poll_ready` wait until the whole buffer is written.

[[crates/transport_mio/src/tcp.rs#TcpFrame]] is the borrowed view. TCP is stream-oriented so both `sequence()` and `stream_id()` return zero; SoupBinTCP or any wire framer above handles record boundaries.
